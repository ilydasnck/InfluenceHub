  ...
  async handlePublishMedia(req: Request, res: Response): Promise<void> {
    ...
    if (!configResult.ok) {
      res
        ...
        .json({ error: configResult.error.message });
      return;
    }

    const { mediaType, mediaUrl, caption } =
      req.body as Partial<PublishMediaParams>;

    if (!mediaType || !mediaUrl) {
      res
        .status(400)
        .json({ error: "mediaType ve mediaUrl alanları zorunludur" });
      return;
    }
    ...
    if (!result.ok) {
      const status = result.error.isRateLimit
        ? 429
        : errorStatusCode(result.error.code);
      logger.warn(
        `Publish başarısız: ${result.error.message}`,
        "InstagramController",
      );
  ...
  async handleGetMediaStatus(req: Request, res: Response): Promise<void> {
    ...
    if (!result.ok) {
      const status = result.error.isRateLimit
        ...
        : errorStatusCode(result.error.code);
      logger.warn(
        `Status sorgusu başarısız: ${result.error.message}`,
        "InstagramController",
      );
      res.status(status).json({ error: result.error });
      return;
