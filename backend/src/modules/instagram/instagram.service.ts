...
const handleRateLimitError = (error: InstagramApiError): InstagramApiError =>
  error.isRateLimit
    ? { ...error, message: "Instagram API rate limit aşıldı. Lütfen daha sonra tekrar deneyin." }
    : error;
...
export const createInstagramService = (repository: InstagramRepository) => ({
  async publishMedia(
    ...
    logger.info(`Medya paylaşımı başlatılıyor: ${params.mediaType}`, "InstagramService");

    const containerResult = await repository.createMediaContainer(config, params);
    if (!containerResult.ok) {
      logger.error("Container oluşturulamadı", "InstagramService", containerResult.error);
    ...
    const publishResult = await repository.publishMediaContainer(config, containerId);
    if (!publishResult.ok) {
      logger.error("Medya yayınlanamadı", "InstagramService", publishResult.error);
